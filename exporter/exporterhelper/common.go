...
func WithQueue(config QueueSettings) Option {
	return func(o *baseExporter) {
		...
		if config.StorageID != nil {
			queue = internal.NewPersistentQueue[Request](capLimiter, o.signal, *config.StorageID, o.marshaler, o.unmarshaler, o.set)
		} else {
			queue = internal.NewBoundedMemoryQueue[Request](capLimiter)
		}
		o.queueSender = newQueueSender(queue, o.set, config.NumConsumers)
	}
}
...
func WithRequestQueue(cfg QueueConfig, queueFactory QueueFactory) Option {
	return func(o *baseExporter) {
		...
		queue := queueFactory(QueueCreateSettings{
			...
			CapacityLimiter: internal.NewItemsCapacityLimiter[Request](cfg.QueueItemsSize),
			DataType:        o.signal,
		})
		o.queueSender = newQueueSender(queue, o.set, cfg.NumConsumers)
	}
}
