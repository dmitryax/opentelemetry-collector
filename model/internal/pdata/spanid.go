...
// SpanIDFromBytes returns a new SpanID from the given byte array.
func SpanIDFromBytes(bytes [8]byte) SpanID {
...
func (t SpanID) CopyToLogRecord(origLog *otlplogs.LogRecord) {
	origLog.SpanId = t.orig
}

func (t SpanID) CopyToExemplar(origLog *otlpmetrics.Exemplar) {
	origLog.SpanId = t.orig
}
